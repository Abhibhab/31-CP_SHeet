...
void solve() {
    ...
    for (int i = 0; i < a; i++) {
        if (i == 0) {
            int ans1 = vec[i + 1] - vec[i];
            int ans2 = vec[a - 1] - vec[i];
            ans = max({ans1, ans2, ans});

        } else if (i == a - 1) {
            int ans1 = vec[i - 1] - vec[i];
            int ans2 = (vec[i] - vec[0]);
            ans = max({ans1, ans2, ans});

        } else {
            int ans1 = vec[i - 1] - vec[i];
            // int ans2 = vec[i + 1 ] - vec[i];
            int ans3 = (vec[i] - vec[0]);
            debug(ans3);
            int ans4 = vec[a - 1] - vec[i];
            ans = max({ans1,  ans3, ans4, ans});
            debug(ans);
