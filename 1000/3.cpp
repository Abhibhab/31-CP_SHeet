...
void solve() {
    int a, b;
    cin >> a >> b;
    vector<int>veca(a), vecb(a);
    for (auto &it : veca)cin >> it;
    for (auto &it : vecb)cin >> it;
    vector<pair<int , int>>vec;
    for (int i = 0; i < a; i++) {
        vec.pb({vecb[i], veca[i]});
    }
    sort(all(vec));
    int count = 1;
    int ans = b;
    int i = 0;
    while (count < a) {
        int option = vec[i].first;
        debug(option);
        if (option < b) {
            int cando = vec[i].second;
            if (count + cando <= a) {
                count += cando;
                ans += option * cando;
                i++;
